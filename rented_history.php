...
$ObjHistCounter = 0;
$isreturn = 1;
               foreach($db->query("select * from tb_history INNER JOIN tb_member ON tb_history.Mid = tb_member.Mid   INNER JOIN  tb_objinfo ON tb_objinfo.Oid = tb_history.Oid  ORDER BY Hid desc ") as $row){
#print_r($row);
                    ...
                    switch($row['isreturn']){
                        case "0":
                            ...
                            $str2 = '<div id="obj'.$row['Oid'].'time" class="obj_time">最晚歸還：'.$Date2.'</div>';
                                if(isset($row['acept'])){
                                   $return_button = '<div id="obj'.$row['Oid'].'retrun" class="objbtn" onclick="return_obj(\''.$row['Oid'].'\',\''.$row['Hid'].'\')">歸還
                                                                </div></li>';
                                }else{
                                $return_button = '</li>';
                                }
